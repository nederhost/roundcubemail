...
class message_highlight extends rcube_plugin
{
  ...
  function mh_preferences($args) {
    if($args['section'] == 'mh_preferences') {
      ...
      $args['blocks']['mh_preferences'] =  array(
        'options' => array(),
        'name'    => rcube_utils::rep_specialchars_output($this->gettext('mh_title'), 'html')
        );
  ...
  function mh_get_form_row($header = 'from', $input = '', $color = '#ffffff', $delete = false) {
    ...
    $header_select->add(rcube_utils::rep_specialchars_output($this->gettext('subject'), 'html'), 'subject');
    $header_select->add(rcube_utils::rep_specialchars_output($this->gettext('from'), 'html'), 'from');
    $header_select->add(rcube_utils::rep_specialchars_output($this->gettext('to'), 'html'), 'to');
    $header_select->add(rcube_utils::rep_specialchars_output($this->gettext('cc'), 'html'), 'cc');
    ...
    $content =  $header_select->show($header) .
      html::span('mh_matches', rcube_utils::rep_specialchars_output($this->gettext('mh_matches'), 'html')) .
      $input->show() .
      html::span('mh_color', rcube_utils::rep_specialchars_output($this->gettext('mh_color'), 'html')) .
  ...
  function mh_preferences_section($args) {
    ...
    $args['list']['mh_preferences'] = array(
      'id'      => 'mh_preferences',
      'section' => rcube_utils::rep_specialchars_output($this->gettext('mh_title'), 'html')
      );
    return($args);
  }
  ...
  function mh_save($args) {
    ...
    $header  = rcube_utils::get_input_value('_mh_header', RCUBE_INPUT_POST);
    $input   = rcube_utils::get_input_value('_mh_input', RCUBE_INPUT_POST);
    $color   = rcube_utils::get_input_value('_mh_color', RCUBE_INPUT_POST);
